...
describe("Test full token timelock flow", function () {
  ...
  beforeEach(async function () {
    ...
    timelock = await helpers.deployTokenTimelock(deployer, camoToken.address);
  ...
  describe("Once deployed", function () {
    it("Only owner can call lock", async function () {
      await expectRevert(
        helpers.lock(beneficiary1, timelock, beneficiary2Address, "0", "0"),
        "TokenTimelock: this function can only be called by the owner"
      );
    });

    it("Rejects a release time in the past", async function () {
      const pastReleaseTime: number = (await time.latest()).sub(
    ...
    it("Rejects amount that is greater than the contract balance", async function () {
      const releaseTime: number = (await time.latest()).add(
        time.duration.years(1)
      );
      await expectRevert(
        helpers.lock(
        ...
        "TokenTimelock: amount is greater than the contract balance"
      );
    });
    ...
    it("Rejects amount that is 0 or less", async function () {
      ...
      await expectRevert(
        ...
        "TokenTimelock: amount is 0 or less"
      );
    });
    ...
    it("Locks tokens for beneficiary", async function () {
      ...
      const beneficiaryData: helpers.BeneficiaryData =
        await helpers.getBeneficiary(deployer, timelock, beneficiary1Address);
    ...
    it("Locks tokens for multiple beneficiaries", async function () {
      ...
      await helpers.lock(
        ...
        timelock,
        beneficiary2Address,
        releaseTime2.toString(),
        amount2
      );
      const beneficiaryData1: helpers.BeneficiaryData =
        await helpers.getBeneficiary(deployer, timelock, beneficiary1Address);
      const beneficiaryData2: helpers.BeneficiaryData =
        await helpers.getBeneficiary(deployer, timelock, beneficiary2Address);
    ...
    it("Rejects release before time limit", async function () {
      ...
      await expectRevert(
        helpers.release(deployer, timelock, beneficiary1Address),
    ...
    it("Releases tokens for beneficiary", async function () {
      ...
      const deletedBeneficiaryData: helpers.BeneficiaryData =
        await helpers.getBeneficiary(deployer, timelock, beneficiary1Address);
      expect(
        +(await helpers.getBalance(camoToken, beneficiary1Address))
      ).to.equal(+amount);
